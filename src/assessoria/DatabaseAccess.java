...
public class DatabaseAccess {
    ...
    public ResultSet select6() {
        ...
        try{
            ...
            return st.executeQuery("SELECT FESTA.NOTA_FISCAL AS NF, CLIENTE.NOME AS CLIENTE, LOCAL.CIDADE, FESTA.DATA_HORA AS DATA, COUNT(MESA.NUMERO) AS NRO_MESAS FROM FESTA JOIN CLIENTE ON (FESTA.CLIENTE=CLIENTE.CPF) JOIN LOCAL ON (FESTA.LOCAL = LOCAL.ID) LEFT JOIN MESA ON (MESA.LOCAL = LOCAL.ID) GROUP BY FESTA.NOTA_FISCAL, LOCAL.CIDADE, CLIENTE.NOME, FESTA.DATA_HORA");
        }
        catch(Exception e){
			e.printStackTrace();
        }
